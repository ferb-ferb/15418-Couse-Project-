...
void compute_forces() {
  ...
  for (Particle &particle_i : fluid_particles) {
    ...
    // Sum the repulsive forces from BOUNDARY particles
    for (const Particle &particle_j : boundary_particles) {
      float dx = particle_i.x - particle_j.x;
      float dy = particle_i.y - particle_j.y;
      float dz = particle_i.z - particle_j.z;
      ...
      // THE TRICK: Use the fluid's own pressure and density for the boundary!
      float fluid_rho_sq = std::max(particle_i.rho * particle_i.rho, EPS);

      // The boundary mirrors the fluid's pressure...
      float boundary_pressure = particle_i.p;

      // SAFETY NET: If the fluid is under-pressurized (like a falling droplet),
      // we force the wall to exert a baseline repulsive pressure so the droplet
      // doesn't ghost through the geometry.
      if (boundary_pressure < 1e-4f) {
        // A baseline pressure based on 10% of the rest density
        boundary_pressure = GAS_CONST * (REST_DENS * 0.1f);
      }

      float pressure_term = -MASS * (particle_i.p / fluid_rho_sq +
                                     boundary_pressure / fluid_rho_sq);
      pressure_fx += pressure_term * grad_coeff * dir_x;
      pressure_fy += pressure_term * grad_coeff * dir_y;
      pressure_fz += pressure_term * grad_coeff * dir_z;

      // (Optional) You can add viscosity here for "no-slip" wall friction later
    }
