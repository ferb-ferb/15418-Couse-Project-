...
// Add a solid 3D block of boundary particles
static void add_cup_volume(const Cup &cup, float local_x_min, float local_x_max,
                           float local_y_min, float local_y_max, float z_min,
                           float z_max, int kind) {

  // Loop across the entire 3D volume using the FLUID's exact spacing
  for (float local_x = local_x_min;
    ...
    for (float local_y = local_y_min;
      ...
      for (float z = z_min; z <= z_max + 0.5f * INITIAL_PARTICLE_SPACING;
        ...
        // Convert the local X/Y to world space
        float world_x;
        float world_y;
        cup_local_to_world(cup, local_x, local_y, world_x, world_y);

        // Add the solid boundary particle
...
// Add all volumetric boundary particles for one cup
static void add_cup_boundary_volumes(const Cup &cup, int kind) {

  // Build the outer bounds
  ...
  // 1. Bottom Slab (Spans the entire width and depth, up to inner_bottom)
  add_cup_volume(cup, outer_left, outer_right, outer_bottom, inner_bottom,
                 outer_front, outer_back, kind);

  // 2. Left Wall (Sits on top of the bottom slab)
  add_cup_volume(cup, outer_left, inner_left, inner_bottom, outer_top,
                 outer_front, outer_back, kind);

  // 3. Right Wall (Sits on top of the bottom slab)
  add_cup_volume(cup, inner_right, outer_right, inner_bottom, outer_top,
                 outer_front, outer_back, kind);

  // 4. Front Wall (Sits on top of bottom slab, fits snugly between left and
  // right walls to avoid overlapping particles)
  add_cup_volume(cup, inner_left, inner_right, inner_bottom, outer_top,
                 outer_front, inner_front, kind);

  // 5. Back Wall (Sits on top of bottom slab, fits snugly between left and
  // right walls)
  add_cup_volume(cup, inner_left, inner_right, inner_bottom, outer_top,
                 inner_back, outer_back, kind);
}
// Add one cup wall surface
// static void add_cup_surface(const Cup &cup, float local_x_min, float
// local_x_max, float local_y_min, float local_y_max, float z_min, float z_max,
// int kind) {
//
//     // Loop across the cup surface
//     for (float local_x = local_x_min; local_x <= local_x_max + 0.5f *
//     CUP_RENDER_SPACING; local_x += CUP_RENDER_SPACING) {
//         for (float local_y = local_y_min; local_y <= local_y_max + 0.5f *
//         CUP_RENDER_SPACING; local_y += CUP_RENDER_SPACING) {
//             for (float z = z_min; z <= z_max + 0.5f * CUP_RENDER_SPACING; z
//             += CUP_RENDER_SPACING) {
//
...
void rebuild_boundary_particles_for_export() {
  ...
  // Add the source cup solid volumes
  add_cup_boundary_volumes(source_cup, 1);

  // Add the receiver cup solid volumes
  add_cup_boundary_volumes(receiver_cup, 2);
  // // Clear the old boundary particles
  // boundary_particles.clear();
  //
